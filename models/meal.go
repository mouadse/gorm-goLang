...
type Meal struct {
	...
	UserID    uuid.UUID      `gorm:"type:uuid;not null;index:idx_meals_user_date_type,priority:1,where:deleted_at IS NULL;index:idx_meals_date_user,priority:2,where:deleted_at IS NULL" json:"user_id"`
	...
	Date      time.Time      `gorm:"type:date;not null;index:idx_meals_user_date_type,priority:2,where:deleted_at IS NULL;index:idx_meals_date_user,priority:1,where:deleted_at IS NULL" json:"date"`
